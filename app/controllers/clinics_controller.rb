class ClinicsController < ApplicationController
  ...
  # GET /clinics/1.json
  def show
    @clinic_comments = ClinicComment.all
  end
  ...
  def comment
    ClinicComment.create(article_id: params[:id], message: params[:comment])
    redirect_to :root
  end
  private
    ...
    def clinic_params
      params.require(:clinic).permit(:title, :content, :image)
    end
end
